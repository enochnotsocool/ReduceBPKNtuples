...
void reduce_Samples_step2()
{
    ...
    /*
    do_reduce("Tgamma450GeV",0);
    do_reduce("Tgamma500GeV",0);
    do_reduce("Tgamma550GeV",0);
    do_reduce("Tgamma600GeV",0);
    do_reduce("Tgamma650GeV",0);
    do_reduce("Tgamma700GeV",0);
    do_reduce("Tgamma750GeV",0);
    */
///*
...
{      
    ...
    sprintf(buffer,"./REDUCE_DATA_Summer12_%s_trg/%s_%s.root",sample_style_.c_str(),sample_style_.c_str(),datatag);
	...
	for(int i=0;i<PhotonInfo.Size;i++) { 
		if(PhotonInfo.Pt[i]<=20) continue;
		Nphotons++;
	}
...
/*
        ...
        int lep_numb = 0;
        if(sample_style==1||sample_style==3||sample_style==4||sample_style==5){
...
///*
        bool accept = false;
        if(sample_style==4 || sample_style==6){
                    //DoublePh
            /*
